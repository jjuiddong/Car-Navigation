...
REALF gis::ArcLengthOfMeridian (REALF phi) {
	REALF alpha, beta, gamma, delta, epsilon, n;
	REALF result;
...
REALF gis::UTMCentralMeridian(int zone) {
	REALF cmeridian;
  cmeridian = DegToRad(-183.0 + ((REALF)zone * 6.0));
...
REALF gis::FootpointLatitude(REALF y) {
	REALF y_, alpha_, beta_, gamma_, delta_, epsilon_, n;
	REALF result;
...
void gis::MapLatLonToXY (REALF phi, REALF lambda, REALF lambda0, REALF &x, REALF &y) {
	REALF N, nu2, ep2, t, t2, l;
	REALF l3coef, l4coef, l5coef, l6coef, l7coef, l8coef;
    //REALF tmp; // Unused
...
void gis::MapXYToLatLon (REALF x, REALF y, REALF lambda0, REALF& phi, REALF& lambda)
{
  REALF phif, Nf, Nfpow, nuf2, ep2, tf, tf2, tf4, cf;
  REALF x1frac, x2frac, x3frac, x4frac, x5frac, x6frac, x7frac, x8frac;
  REALF x2poly, x3poly, x4poly, x5poly, x6poly, x7poly, x8poly;
...
int gis::LatLonToUTMXY (REALF lat, REALF lon, int zone, REALF& x, REALF& y) {
  if ( (zone < 1) || (zone > 60) )
  ...
  //x = x * UTMScaleFactor + 600000.0f;
  //y = y * UTMScaleFactor + 200000.0f;
  if (y < 0.0)
    y = y + 10000000.0;
...
// Returns:
// The function does not return a value.
void gis::UTMXYToLatLon (REALF x, REALF y, int zone, bool southhemi, REALF& lat, REALF& lon) {
  REALF cmeridian;
